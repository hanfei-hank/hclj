...
instance Show n => Show (Term n) where
    ...
    show (TTable n _ _ _ _) = "(TTable " <> toString n <> " )"
...
instance Monad Term where
    ...
    TTable {..} >>= f = TTable _tTableName _tModule _tMeta (fmap (fmap (>>= f)) _tFields) _tInfo
...
typeof t = case t of
      ...
      TBinding {..} -> case _tBindType of
        BindLet -> Left "let"
      TTable {..} -> Right $ TySchema TyTable TyAny
