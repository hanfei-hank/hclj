...
-- |
-- Module      :  Seal.Lang.Clj.Types.Typecheck
--
-- Types for clj typechecker.
--
...
module Seal.Lang.Clj.Types.Typecheck
  (
    ...
    Special (..),
    Fun (..),fInfo,fName,fTypes,fType,fArgs,fBody,
    Node (..),aId,aTy,
    Named (..),
    AstBindType (..),
    AST (..),aNode,aAppFun,aAppArgs,aBindings,aBody,aBindType,aList,aObject,aPrimValue,
    Visit(..),Visitor
  ) where
...
instance Show UserType where
  show Schema {..} = "{" ++ toString (toText _utModule) ++ "." ++ toString (toText _utName) ++ " " ++ show _utFields ++ "}"
...
instance Show TcId where show TcId {..} = toString _tiName ++ show _tiId
