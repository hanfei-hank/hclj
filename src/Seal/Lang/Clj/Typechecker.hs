...
-- |
-- Module      :  Seal.Lang.Clj.Typechecker
--
-- Stylized static typechecker for clj, operates on
...
-- function.
--
-- clj's support for overloaded native functions is one reason
-- a more traditional HM approach is not used.
--
-- TODO: handle lazy functions better (map, filter etc).
--

module Seal.Lang.Clj.Typechecker where
...
import Seal.Lang.Clj.Types.Typecheck
import Seal.Lang.Clj.Types.Runtime
import Seal.Prelude
import qualified Universum.Unsafe as Unsafe


throwCE :: MonadThrow m => Info -> String -> m a
throwCE i s = throwM $ CheckerException i s

die' :: MonadThrow m => TcId -> String -> m a
die' i = throwCE (_tiInfo i)
...
solveOverloads = do
  ...
  forM_ (M.toList done) $ \(i,o) -> case _oSolved o of
      Nothing -> do
        ...
        addFailure i $ "Unable to solve overloaded function: " ++ toString (_oFunName o)
      Just {} -> return ()
...
asPrimString t = throwCE (_tiInfo (_aId (_aNode t))) $ "Expected literal string: " ++ show t
...
processNatives Pre a@(App i FNative {..} argASTs) = do
  case _fTypes of
    ...
    orgFunType@FunType {} :| [] -> do
      ...
      _ <- (\f -> zipWithM f (_ftArgs mangledFunType) argASTs) $ \(Arg _ argTy _) argAST -> case (argTy,argAST) of
...
inlineAppArgs appNode defunType defunArgs defunBody args = do
  ...
  letId <- freshId appInfo (toText $ show bindType)
...
scopeToBody i args bod = do
  ...
  case bt of
    ...
    _ -> throwCE i "Malformed def body"
...
idTyVar :: TcId -> Type n
idTyVar i = mkTyVar (TypeVarName $ toText $ show i) []
...
mangle i = over (tyVar.tvName.typeVarName) (pfx (toText $ show i))
...
toFun (TVar (Left (Direct TNative {..})) _) = do
  ...
  return $ FNative _tInfo (toText  _tNativeName) ft' 
toFun (TVar (Left (Ref r)) _) = toFun (fmap Left r)
toFun (TVar Right {} i) = throwCE i "Value in fun position"
toFun TDef {..} = do -- TODO currently creating new vars every time, is this ideal?
  let fn = toText  _tModule <> "." <> toText  _tDefName
...
toAST TBinding {..} = do
  bi <- freshId _tInfo (toText $ show _tBindType)
  bn <- trackIdNode bi
  bs <- forM _tBindPairs $ \(Arg n t ai,v) -> do
    aid <- freshId ai (pfx (toText $ show bi) n)
...
toAST TModule {..} = throwCE _tInfo "Modules not supported"
toAST TUse {..} = throwCE _tInfo "Use not supported"


trackPrim :: Info -> PrimType -> PrimValue -> TC (AST Node)
trackPrim inf pty v = do
  let ty :: Type UserType = TyPrim pty
  Prim <$> (trackNode ty =<< freshId inf (toText $ show ty) ) <*> pure v
...
toUserType t = case t of
  ...
  _ -> throwCE (_tInfo t) $ "toUserType: expected user type: " ++ show t
  where
    derefUT (Ref r) = toUserType' (fmap Left r :: Term (Either Ref n))
    derefUT Direct {} = throwCE (_tInfo t) $ "toUserType: unexpected direct ref: " ++ show t

toUserType' :: Show n => Term (Either Ref n) -> TC UserType
toUserType' t = throwCE (_tInfo t) $ "toUserType: expected user type: " ++ show t

bindArgs :: Info -> [a] -> Int -> TC a
bindArgs i args b =
  case args `atMay` b of
    Nothing -> throwCE i $ "Missing arg: " ++ show b ++ ", " ++ show (length args) ++ " provided"
    Just a -> return a
...
mkTop t@TConst {..} = do
  ...
  TopConst _tInfo (toText  _tModule <> "." <> _aName _tConstArg) <$>
    traverse toUserType (_aType _tConstArg) <*>
    toAST (_cvRaw _tConstVal) <*> pure (_mDocs _tMeta)
mkTop t = throwCE (_tInfo t) $ "Invalid top-level term: " ++ abbrev t
...
resolveTy rt = do
  ...
  case runState (go rt) [] of
    (Just r,_) -> return r
    (Nothing,rs) -> throwCE def $ "Error: recursive type vars: " ++ show rs
...
resolveAllTypes = do
  ast2Ty <- use tcAstToVar >>= \a2v -> (`M.traverseWithKey` a2v) $ \i tv -> do
    ...
    case tysm of
      Nothing -> throwCE def $ "resolveAllTypes: untracked type var: " ++ show (i,tv)
...
typecheckTopLevel (Ref r) = do
  ...
  debug $ "===== Done: " ++ abbrev r
  return tl'
typecheckTopLevel (Direct d) = throwCE (_tInfo d) $ "Unexpected direct ref: " ++ abbrev d
